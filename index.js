...
app.get("/", async (req, res) => {
  ...
  try {
    ...
    let captchaAns = captchaMatch
      ? parseInt(captchaMatch[1]) + parseInt(captchaMatch[2])
      : 10;
    ...
    // --- Numbers API ---
    if (type === "numbers") {
      const numResp = await makeRequest(
        "GET",
        `${CONFIG.baseUrl}/agent/res/data_smsnumbers.php?frange=&fclient=&sEcho=2&iColumns=8&sColumns=%2C%2C%2C%2C%2C%2C%2C&iDisplayStart=0&iDisplayLength=-1`,
        null,
        { "Referer": `${CONFIG.baseUrl}/agent/MySMSNumbers`, "X-Requested-With": "XMLHttpRequest" }
      );
      ...
      try {
        ...
        if (jsonData.aaData && Array.isArray(jsonData.aaData)) {
          jsonData.aaData = jsonData.aaData.map((row) => {
            ...
            if (cleanRow[3] && cleanRow[3].includes("<br")) {
    ...
    // --- SMS API ---
      ...
      const reportPage = await makeRequest(
        "GET",
        `${CONFIG.baseUrl}/agent/SMSCDRReports`,
        null,
      ...
      const smsUrl = `${CONFIG.baseUrl}/agent/res/data_smscdr.php?fdate1=${encodeURIComponent(fdate1)}&fdate2=${encodeURIComponent(fdate2)}&frange=&fclient=&fnum=&fcli=&fg=0&csstr=${csstrToken}&sEcho=1&iDisplayLength=100`;

      const smsResp = await makeRequest(smsUrl, "GET", null, {
